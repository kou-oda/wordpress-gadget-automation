...
このガイドでは、`data/products.json`の商品データを手動で更新する方法を説明します。

## なぜ手動更新が必要か？

Amazon PA-APIには以下の制限があります：
- **リクエスト制限**: 1秒あたり1リクエスト
- **レート制限**: 短時間に多数のリクエストを送信すると制限される
- **100個の商品取得には時間がかかる**: 最低でも100秒（約1分40秒）必要

そのため、PA-APIを使用した自動更新ではなく、定期的な手動更新を推奨します。
